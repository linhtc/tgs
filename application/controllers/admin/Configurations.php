...
class Configurations extends MY_Controller {
    ...
    public function index() {
        ...
        $listJs = array(
            ...
            '/static/admin/js/extra/configurations.js'
        );
...
//        $permission = $this->check_permission($this->class, 'view');
        ...
        $sortMaps = array('id', 'id', 'apply_key', 'apply_name', 'apply_value', 'apply_value2');
        $order = $this->input->post('order', true);
        $sort = isset($order[0]['column']) ? $order[0]['column'] : 0;
        $type = isset($order[0]['dir']) ? $order[0]['dir'] : 'desc';
        $start = $this->input->post('start', true);
        $query = $this->db->select('`id`, `apply_key` k, `apply_name` n, `apply_value` v1, `apply_value2` v2')->from($this->configModel)->where('deleted', 0);
        $this->criteria($query);
        ...
        if($num){
            ...
            foreach($list as $index=>$item){
                array_push($set, array(null, $item->id, $item->k, $item->n, $item->v1, $item->v2));
            }
