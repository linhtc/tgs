...
$(document).ready(function($) {
	...
	$('nav ul li a').click(function(){
        ...
        if(el.toString().indexOf('design') >= 0){
        	var tmpEl = el === '#design' ? $(that).parent().find('ul') : $(that).parent().parent().parent().parent().find('ul');
        	if(tmpEl.length >= 1){
                tmpEl.css({'display': 'none', 'position': 'fixed', 'opacity': 0, 'left' : 0, 'top': 0});
                setTimeout(function(){
                    tmpEl.css({'display': 'none', 'position': 'absolute', 'opacity': 1});
                }, 1000)
			}
		}
        return false;    
    ...
    $("#design-sheet-metals").owlCarousel({
        autoPlay: 3200,
        items : 5,
        navigation: true,
        itemsDesktop : [1600,4],
	...
	$('.form-contact').submit(function(event) {  
        ...
        var datos = {
            usr_name : $('#name').val(),
            usr_email: $('#email').val(),
            usr_message: $('#message').val()
        };
	    // var datos = $(this).serialize();

	    // console.log(url);
	    // console.log(datos);
        $('#contact-button').notify('Wating...', {className: 'warning', position: 'right top'});
	    $.post(url, datos, function(result) {
	    	// $('.result').html(resultado);
			if(result.status === 1){
                $('#contact-button').notify('OK', {className: 'success', position: 'right top'});
                $('#name').val('');
                $('#email').val('');
                $('#message').val('');
			} else{
                $('#contact-button').notify('Error', {className: 'error', position: 'right top'});
			}
		});  
 	});  
