...
(function (factory) {
    ...
    $.jstree.plugins.table = function(options,parent) {
        ...
        this._edit = function (obj, col, element) {
            ...
            if (element) {
                element = $(element);
                if (element.prop("tagName").toLowerCase() === "div" && col.value !== 'prove_report' && col.value !== 'prove_email') {
            ...
            var rtl = this._data.core.rtl,
                ...
                h2 = null;
                if(col.value !== 'prove_report' && col.value !== 'prove_email'){
                    h2 = $("<"+"input />", {
                        "value" : t,
                    ...
                    var iModalID = '#infoModal';
                    $(iModalID).modal('show');
                    $(iModalID).find('.modal-content').block({
                        message: '<i class="fa fa-spinner fa-spin fa-2x text-white"></i>',
                        css: {backgroundColor: 'none', border: 'none'}
                    });
                    $(iModalID).on('shown.bs.modal', function (e) {
                        if(editorStatic === null){
                            var toolbar = [
                                // 'title',
                                'bold',
                                'italic',
                        ...
                        editorStatic.setValue(t);
                        $(iModalID).find('.modal-content').unblock();
                    });
                }
        ...
        this._prepare_table = function (obj, callback) {
                    ...
                    return function (e) {
                        if (gs.context) {
                            ...
                            $.vakata.context.show(this,{ 'x' : e.pageX, 'y' : e.pageY },{
                                "edit":{label:"Sửa nội dung","action": function (data) {
                                        var obj = t.get_node(node);
