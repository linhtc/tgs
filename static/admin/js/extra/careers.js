                        ...
                        success: function (result) {
                            if (result.status === 1) {
                                ...
                                Object.keys(item).forEach(function (key) {
                                    ...
                                    if(key === 'employee_skills'){
                                        try{
                                            let skills = JSON.parse(item[key]);
                                            let index = 0;
                                            for(let skill in skills){
                                                if(skills.hasOwnProperty(skill)){
                                                    $('#skill_'+(++index)).val(skill+': '+skills[skill].toString());
                                                }
                                            }
                                        } catch (e) {
...
function responsive_filemanager_callback(field_id){
    ...
    let bgID = '#employer_avatar';
    if(url !== undefined && url !== ''){
        $(bgID).parent().find('label').addClass('mdc-floating-label--float-above');
        $(bgID).val('/media/filemanager/source/'+url);
    } else{
        $(bgID).parent().find('label').removeClass('mdc-floating-label--float-above');
        $(bgID).val('');
    }
    _dialogFile.close();
}
...
$(document).ready(function () {
    ...
    _dialogSkill = new mdc.dialog.MDCDialog(document.querySelector('#mdc-dialog-skills'));
    _infoTable = $('#infoTable').DataTable({
        ...
        drawCallback: function(settings){
            $("#infoTable tr").each(function(){
                $(this).find("td:eq(3)").on('click', function(){
                    $('#lightbox-anchor').attr('href', _infoTable.cell(this).data()).click();
                });
                $(this).find("td:eq(5)").on('click', function(){
                    let skillTableID = '#skillTable';
                    $(skillTableID).html(_infoTable.cell(this).data());
                    _dialogSkill.show();
                });
                $(this).find("td:eq(7)").on('click', function(){
                    try{
                        let idx = _infoTable.cell(this).index();
                        let row = _infoTable.rows( idx.row ).data();
                        let skills = JSON.parse(row[0][7]);
                        let skillTableID = '#skillTable';
                        $(skillTableID).html('<table class="mdl-data-table table"></table>');
                        for(let skill in skills){
                            if(skills.hasOwnProperty(skill)){
                                $(skillTableID).find('table').append('<tr><td>'+skill+'</td><td>'+skills[skill]+'</td></tr>');
                            }
                        }
                        _dialogSkill.show();
                    } catch (e) {
